...
const AvatarPicker = ({ name, email, selectedStyle, onSelect }) => {
    const seed = name?.trim() || email?.trim() || 'New user'

    return (
        <div>
            <div className='mb-2 flex items-center justify-between gap-3'>
                <label className='block text-[12px] font-medium uppercase tracking-[0.04em] text-[#5f5e5a]'>Avatar</label>
                <span className='text-[11px] font-medium text-[#888780]'>Generated by DiceBear</span>
            </div>
            <div className='grid grid-cols-3 gap-2'>
                {avatarStyles.map(option => {
                    ...
                    return (
                        ...
                        >
                            <img
                                src={buildAvatarUrl({ style: option.id, seed })}
                                alt=''
